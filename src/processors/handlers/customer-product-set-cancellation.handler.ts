...
export class CustomerProductSetCancellationHandler implements EventHandler {
  readonly topic = "customer.product.set_cancellation";
  ...
  async handle(event: VottEvent, ctx: HandlerContext): Promise<void> {
    ...
    await ctx.subscriptions.setCancellation(
