-- Add cancelled_today to analytics.mv_dashboard for executive "today" KPIs.
-- Keeps stock definitions from migration 032; adds UTC-day cancellation count.
...
subs as (
  select
    ...
    count(*) filter (
      where cancelled_at is not null
        and cancelled_at::date = (timezone('utc', now()))::date
    )::bigint as cancelled_today
...
comment on materialized view analytics.mv_dashboard is
  'KPI snapshot. Active=distinct open paid customers; Trials=open free trials; Cancelled=cancelled not expired; cancelled_today=UTC day. Refresh: analytics.refresh_dashboard().';
