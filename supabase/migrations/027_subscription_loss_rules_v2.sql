-- Phase 9.5d: Updated Combined Loss rules (Subscriptions + Trials).
-- Research / Vimeo-aligned SQL:
--   Web: set_cancellation | expired | (charge_failed AND subscription_status = expired)
--   Non-web: cancelled | expired | disabled
--   Trials (any platform): free_trial_expired → trial_expired
...
with base as (
  select
    ...
    e.customer_id,
    lower(coalesce(
      nullif(trim(v.subscription_status), ''),
...
comment on view analytics.v_daily_subscription_metrics is
  'Gain/loss from subscription_events. Loss Web=set_cancellation|expired|(charge_failed+expired status); non-Web=cancelled|expired|disabled; Trial=trial_expired.';

-- Reprocess: include charge_failed only when status is expired (matches counting rule)
...
comment on function public.fn_unprocessed_loss_events(date, date, integer) is
  'Unprocessed loss-related vott_events incl. web charge_failed when subscription_status=expired.';
...
as $$
  ...
  pending as (
    select count(*)::bigint as n
    from public.fn_unprocessed_loss_events(p_date, p_date, 100000)
  )
  select
    vott.n,
    se.n,
