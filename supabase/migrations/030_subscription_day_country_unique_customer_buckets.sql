-- Phase 9.5g: expose distinct-customer bucket counts for day-by-country gain/loss.
-- Must DROP+CREATE: CREATE OR REPLACE VIEW cannot insert/rename columns mid-list.

drop function if exists analytics.fn_subscription_metrics_day_country(date, date, text);
drop view if exists analytics.v_daily_subscription_country_metrics;

create view analytics.v_daily_subscription_country_metrics as
with base as (
  select
...
select
  ...
  -- Keep original unique_customers_* positions from migration 029, then append buckets
...
comment on view analytics.v_daily_subscription_country_metrics is
  'Day-by-day country gain/loss from subscription_events.event_created_at (UTC), with both event counts and distinct-customer counts for combined/subscription/trial buckets.';

create function analytics.fn_subscription_metrics_day_country(
  p_start_date date,
  p_end_date date,
...
as $$
  select
    ...
    v.unique_subscription_gain,
    v.unique_subscription_loss,
    v.unique_trial_gain,
    v.unique_trial_loss
